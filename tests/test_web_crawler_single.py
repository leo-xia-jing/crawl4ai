...
class TestWebCrawler(unittest.TestCase):
    ...
    def test_unsupported_extraction_strategy(self):
        with self.assertRaises(Exception) as context:
            self.crawler.run(url=TEST_URL, extraction_strategy="UnsupportedStrategy", bypass_cache=True)
        self.assertIn("Unsupported extraction strategy", str(context.exception))
    
    def test_invalid_css_selector(self):
        with self.assertRaises(ValueError) as context:
            self.crawler.run(url=TEST_URL, css_selector="invalid_selector", bypass_cache=True)
